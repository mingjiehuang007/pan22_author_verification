...
def train_decider(checkpoint_path):
    train_sents_r = np.load(checkpoint_path + "/train_sents_rs.npy", mmap_mode='r')
    train_labels = np.array(labels[:train_data_len])
    valid_sents_r = np.load(checkpoint_path + "/test_sents_rs.npy", mmap_mode='r')
    valid_labels = np.array(labels[train_data_len:])

    weight_path = checkpoint_path + '/weights1.h5'
    callbacks = [
        ModelCheckpoint(
            weight_path,
    ...
    model.fit(train_sents_r, train_labels, epochs=500, validation_data=(valid_sents_r, valid_labels),
              callbacks=callbacks)
    model.save(checkpoint_path + '/final_model.h5')
...
if __name__ == '__main__':
    ...
    checkpoint_path_1 = "checkpoint1"
    checkpoint_path_2 = "checkpoint2"
    checkpoint_path_3 = "checkpoint3"
    train_decider(checkpoint_path_1)
    train_decider(checkpoint_path_2)
    train_decider(checkpoint_path_3)
