...
cls_layer = Model(inputs=model.input, outputs=model.layers[-4].output)  #后面会从cls_layer提取cls向量作为句子表示
...
test_pair_count = count_pairs(data[train_data_len:])
...
def get_sents_represent(generator, texts_num, pair_count):
    ...
    for c in pair_count:
        ...
        sent_r = sents_r[start:end]
        sent_r = sent_r.reshape(1, c, 768)
        sent_r = np.mean(sent_r, axis=1, keepdims=True)
        res = np.append(res, sent_r)
        start = end
...
train_sents_rs = get_sents_represent(train_generator, len(data[:train_data_len]), train_pair_count)
test_sents_rs = get_sents_represent(test_generator, len(data[train_data_len:]), test_pair_count)
