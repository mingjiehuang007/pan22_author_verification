...
def get_segments(datas):
    ...
    for data in datas:
        ...
        for i in text1:
            for j in text2:
                new_datas.append((i,j,data[2],data[3]))
...
def main():
    ...
    # test_data_origin = get_test_data(test_path + '/pairs.jsonl')
    test_data_origin = get_test_data(r'D:\NLP\数据集\PAN2022数据集\process_data\pairs.jsonl')

    test_data = get_segments(test_data_origin)
    ...
    # config_path = r'/home/huang22/cased_L-12_H-768_A-12/bert_config.json'
    # checkpoint_path = r'/home/huang22/cased_L-12_H-768_A-12/bert_model.ckpt'
    # dict_path = r'/home/huang22/cased_L-12_H-768_A-12/vocab.txt'
    ...
    # model.load_weights('/home/huang22/AutherVerification2022/checkpoint1/best_model.weights')  #加载微调后的权重

    cls_layer = Model(inputs=model.input, outputs=model.layers[-3].output)

    predict_model = load_model('checkpoint1/final_model.h5')
    # predict_model = load_model('/home/huang22/AutherVerification2022/checkpoint1/final_model.h5')
    ...
    test_pair_count = count_pairs(test_data_origin)
    test_sents_s = get_sents_represent(test_generator, len(test_data_origin), test_pair_count)

    res = predict_model.predict(test_sents_s)
    ...
    with open('answers.jsonl', 'w', encoding='utf-8') as f:
        for index, value in enumerate(test_data_origin):
