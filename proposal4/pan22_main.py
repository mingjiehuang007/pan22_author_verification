...
def main():
    ...
    def get_sents_represent(generator, texts_num):
        ...
        res = sents_r.reshape(texts_num, 1, 768)
    ...
    test_sents_r = get_sents_represent(test_generator, test_pair_num)
    res = predict_model.predict(test_sents_r)
    n_res = np.argmax(res, axis=-1)
    test_sents_ans = combine_prediction(n_res, len(test_data), test_pair_count)
    final_res = final_model.predict(test_sents_ans)
    final_res_list = np.argmax(final_res, axis=-1)
    ...
    with open('answers.jsonl', 'w', encoding='utf-8') as f:
        for index, value in enumerate(test_data):
            dic = {"id": value[2], "value": final_res_list[index]}
