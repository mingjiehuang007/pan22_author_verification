...
def combine_prediction(predict_res, texts_num, pair_count):
    ...
    for c in pair_count:
        ...
        res = np.append(res, count_01)
        start = end

    res = res.reshape(texts_num, 2)
...
train_sents_ans = combine_prediction(train_sents_s, len(data[:train_data_len]), train_pair_count)
valid_sents_ans = combine_prediction(valid_sents_s, len(data[train_data_len:]), valid_pair_count)
